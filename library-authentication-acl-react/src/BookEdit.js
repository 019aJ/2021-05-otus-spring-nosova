...
class BookEdit extends Component {
  ...
  async componentDidMount() {
      ...
      if(!this.isAvailable(book) || !this.isAvailable(authors) || !this.isAvailable(genres)){
          this.setState({item: this.emptyItem, authors: [], genres: [], granted: false});
      }
      else {
        ...
        this.setState({item: book, authors: authors, genres: genres, granted: true});
      }
  }
  ...
  async handleSubmit(event) {
    ...
    }).then(result => {if(result.status == 403){window.alert('Book modification is not permited')}});
  ...
  render() {
    ...
    if(this.state.granted === false){
      return <div>
        <AppNavbar/>
        <Container>
          <h2>{this.props.match.params.id !== 'new' ? 'Update is not permited' : 'Create is not permited' }</h2>
        </Container>
      </div>
    }
